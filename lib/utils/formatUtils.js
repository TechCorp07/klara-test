 ...
 * @param {number} value - Number to format as percentage (0-100 or 0-1)
 * @param {number} decimals - Number of decimal places
 * @param {boolean} convertFromDecimal - Whether to convert from decimal (0-1) to percentage (0-100)
 * @returns {string} Formatted percentage string
 */
export const formatPercentage = (value, decimals = 0, convertFromDecimal = true) => {
  if (value === null || value === undefined) return ""
  ...
  const formattedValue = Number.parseFloat(percentValue).toFixed(decimals)
