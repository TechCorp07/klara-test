...
export function StatusBadge({ status, type = "default", size = "default" }) {
  ...
  const colorClass = statusColors[lowerStatus] || statusColors[type] || statusColors.default
  const sizeClass = sizeClasses[size] || sizeClasses.default

  // Format the status text (capitalize first letter)
  const formattedStatus = typeof status === "string" ? status.charAt(0).toUpperCase() + status.slice(1) : status

  return <span className={`${colorClass} ${sizeClass} rounded-full font-medium`}>{formattedStatus}</span>
}
 ...
 */
export function Modal({ isOpen, onClose, title, children, size = "md", closeOnClickOutside = true, footer = null }) {
    ...
    >
      ...
      <div className="flex items-center justify-center min-h-screen p-4 text-center sm:block sm:p-0">
        ...
        >
          ...
          {/* Footer */}
          {footer && (
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">{footer}</div>
          )}
        </div>
      </div>
    </div>
 ...
 */
export function PageHeader({ title, subtitle, actionLabel, actionUrl, onActionClick, backUrl, backLabel = "Back" }) {
  return (
 ...
 */
export function EmptyState({ message = "No data found", icon: Icon = null, actionLabel, actionUrl, onAction }) {
  return (
 ...
 */
export function FilterBar({ filters, onFilterChange, onSearch, onRefresh }) {
 ...
 */
export function HIPAABanner({ type = "phi", message, icon, lastAccessed = new Date() }) {
    ...
    switch (type) {
      case "security":
        return {
          ...
          message:
            message ||
            "This page contains security settings required for HIPAA compliance. All security changes are logged for compliance purposes.",
        }
      case "audit":
        return {
          icon: icon || FaInfoCircle,
          ...
          message:
            message ||
            "This page provides detailed audit logs of all PHI access events. All access to this information is logged for security and compliance purposes.",
        }
      case "medical":
        return {
          icon: icon || FaInfoCircle,
          ...
          message:
            message ||
            "This page contains protected medical information. Access to this information is logged and monitored for compliance purposes.",
        }
      case "phi":
      default:
        return {
          ...
          message:
            message ||
            "This page contains Protected Health Information (PHI) as defined by HIPAA regulations. Access to this information is logged and monitored for compliance purposes.",
        }
    }
  }

  const { icon, title, message: contentMessage } = getBannerContent()
      ...
      <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
        <div>
          <p className="text-sm text-gray-700">
            Showing <span className="font-medium">{getItemRange()}</span> of{" "}
