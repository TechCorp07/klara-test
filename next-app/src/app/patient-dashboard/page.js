    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/medications" className="text-blue-600 hover:text-blue-800 text-sm">View all</Link>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/health-devices" className="text-blue-600 hover:text-blue-800 text-sm">View all</Link>
          </div>
          
          {wearableData && wearableData.results && wearableData.results.length > 0 ? (
            <div className="space-y-4">
              {Object.entries(groupMetricsByType(wearableData.results)).slice(0, 3).map(([type, metrics]) => {
                const latestMetric = metrics[0];
                return (
                  <div key={type} className="border-b pb-3">
                    <p className="font-medium">{formatMetricType(type)}</p>
                    <p className="text-sm text-gray-600">
                      {latestMetric.value} {latestMetric.unit} - {new Date(latestMetric.timestamp).toLocaleDateString()}
                    </p>
                  </div>
                );
              })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/medical-records" className="text-blue-600 hover:text-blue-800 text-sm">View all</Link>
          </div>
            ...
            <div className="space-y-4">
              {medicalRecords.results.slice(0, 3).map((record) => (
                <div key={record.id} className="border-b pb-3">
                  ...
                  <p className="text-sm text-gray-600">Last updated: {new Date(record.updated_at).toLocaleDateString()}</p>
                </div>
              ))}
            </div>
          ) : (
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Link href="/messages/new" className="bg-green-100 hover:bg-green-200 text-green-800 p-4 rounded-lg text-center">
            <div className="flex flex-col items-center">
              <svg className="h-8 w-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <Link href="/health-devices" className="bg-purple-100 hover:bg-purple-200 text-purple-800 p-4 rounded-lg text-center">
            <div className="flex flex-col items-center">
              <svg className="h-8 w-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <Link href="/medications/refill" className="bg-yellow-100 hover:bg-yellow-200 text-yellow-800 p-4 rounded-lg text-center">
            <div className="flex flex-col items-center">
              <svg className="h-8 w-8 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
