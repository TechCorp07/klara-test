...
import { audit } from '@/lib/services/auditService'; // Import from existing service
...
export default function DashboardClient() {
  ...
  const { data: complianceReports } = useQuery({
    ...
    queryFn: () => audit.getComplianceReports(),
  ...
  const { data: securityIncidents } = useQuery({
    ...
    queryFn: () => audit.getSecurityIncidents(),
    enabled: !!user && (user.role === 'compliance' || user.role === 'admin' || user.role === 'superadmin'),
    onError: (error) => {
  ...
  const { data: complianceMetrics } = useQuery({
    ...
    queryFn: () => audit.getComplianceMetrics(),
    enabled: !!user && (user.role === 'compliance' || user.role === 'admin' || user.role === 'superadmin'),
    onError: (error) => {
