...
export default function CaregiverDashboard() {
  ...
  const { data: patients = { results: [], total_count: 0 } } = useQuery({
  ...
  const { data: appointments = { results: [], upcoming_count: 0 } } = useQuery({
  ...
  const { data: medications = { results: [], today_count: 0 } } = useQuery({
  ...
  // Fetch care tasks
  const { data: careTasks = { results: [], pending_count: 0 } } = useQuery({
  ...
  useEffect(() => {
    if (mounted && user && user.role !== "caregiver") {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <p className="text-3xl font-bold text-blue-600">{patients?.total_count || 0}</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <p className="text-3xl font-bold text-green-600">{appointments?.upcoming_count || 0}</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <p className="text-3xl font-bold text-purple-600">{medications?.today_count || 0}</p>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <p className="text-3xl font-bold text-yellow-600">{careTasks?.pending_count || 0}</p>
            ...
            <div className="space-y-4">
              {appointments.results.slice(0, 5).map((appointment) => (
                <div key={appointment.id} className="border-b pb-3">
                  ...
                  <p className="text-sm text-gray-600">
                    Provider: Dr. {appointment.provider_name} - {appointment.appointment_type}
                  </p>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="space-y-4">
              {medications.results.slice(0, 5).map((medication) => (
                <div key={medication.id} className="border-b pb-3">
                  ...
                  <p className="text-sm text-gray-600">
                    Time: {medication.scheduled_time} - {medication.frequency}
                  </p>
                  {medication.status === "due" && (
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {careTasks?.results && careTasks.results.length > 0 ? (
            <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/resources/caregiver" className="text-blue-600 hover:text-blue-800 text-sm">
            View all
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="border rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600 mb-2">Access guides for providing care for different conditions.</p>
          ...
          <div className="border rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600 mb-2">Connect with other caregivers for support and advice.</p>
          ...
          <div className="border rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600 mb-2">Resources to help caregivers maintain their own health.</p>
