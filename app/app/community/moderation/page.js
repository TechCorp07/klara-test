...
export default function CommunityModeration() {
  ...
  const rejectPostMutation = useMutation({
    mutationFn: ({ postId, reason }) =>
      communication.moderatePost(postId, {
        action: "reject",
        reason,
      }),
    onSuccess: () => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {reportedPosts?.results && reportedPosts.results.length > 0 ? (
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded-full ${
                          post.report_count > 3 ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"
                        }`}
                      >
                  ...
                  {selectedPost.reports &&
                    selectedPost.reports.map((report, index) => (
                      <div key={index} className="text-sm">
                        ...
                        <p>
                          Reported by: {report.reporter_name} on {new Date(report.reported_at).toLocaleString()}
                        </p>
                      </div>
                    ))}
                </div>
              ...
              <div className="border-t pt-6">
                ...
                {filterStatus === "rejected" && (
                  <div className="bg-red-100 text-red-800 px-4 py-3 rounded">
                    ...
                    {selectedPost.moderation_reason && <p className="mt-2">Reason: {selectedPost.moderation_reason}</p>}
                  </div>
                )}
              </div>
            </div>
