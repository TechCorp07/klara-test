...
/**
 * Client component for audit-log page
 */
export default function AuditLogClient() {
  ...
  // Fetch audit events
  const { data: auditEvents, isLoading, error } = useQuery({
    queryKey: ['auditEvents', filters],
    queryFn: () => auditService.getEvents(filters),
    enabled: !!user && (user.role === 'admin' || user.role === 'compliance'),
    onError: (error) => {
  ...
  const exportEventsMutation = useMutation({
    mutationFn: (options) => auditService.exportEvents(options),
    onSuccess: (data) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {auditEvents && auditEvents.results && auditEvents.results.length > 0 ? (
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">1</span> to <span className="font-medium">{auditEvents.results.length}</span> of{' '}
