...
export default function ComplianceClient() {
  ...
  const calculateCompliancePercentage = (category) => {
    ...
    const compliantChecks = category.checks.filter((check) => check.status.toLowerCase() === "compliant").length
            ...
            <div className="flex flex-col md:flex-row items-center justify-between">
              <div>
                ...
                <p className="text-gray-600">
                  Last updated: {new Date(complianceReport.last_updated).toLocaleString()}
                </p>
              </div>
              <div className="mt-4 md:mt-0 text-center">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  {complianceReport.overall_score >= 90
                    ? "Excellent"
                    : complianceReport.overall_score >= 75
                      ? "Good"
                      : complianceReport.overall_score >= 60
                        ? "Fair"
                        : "Poor"}
                </p>
              </div>
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
                  ...
                  <div className="mt-4 md:mt-0 flex items-center">
                    ...
                    <span className="text-sm font-medium text-gray-900">
                      {calculateCompliancePercentage(category)}%
                    </span>
                  </div>
                </div>
                      ...
                      {category.checks &&
                        category.checks.map((check, checkIndex) => (
                          <tr key={checkIndex}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(check.status)}`}
                              >
