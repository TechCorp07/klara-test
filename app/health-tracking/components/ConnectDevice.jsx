...
export default function ConnectDevice({ wearableTypes, onConnect, isPending }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {selectedWearable && (
        <div className="mt-4 p-4 bg-gray-50 rounded-md">
          <div className="flex items-start">
            <div
              className={`flex-shrink-0 h-10 w-10 rounded-md ${getDeviceColor(selectedWearable)} flex items-center justify-center`}
            >
            ...
            <div className="ml-4">
              ...
              <p className="mt-1 text-sm text-gray-500">{connectionDetails?.description}</p>
      ...
      <div className="mt-8">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Supported Wearable Devices</h3>
            ...
            >
              <div className="flex items-center justify-center h-12 mb-4">
                <div className={`h-10 w-10 rounded-md ${getDeviceColor(wearable.id)} flex items-center justify-center`}>
                  <span className="text-lg font-semibold">{wearable.name.charAt(0)}</span>
                </div>
              </div>
              <h4 className="text-center text-sm font-medium text-gray-900">
                {wearable.name}
                {wearable.mobileOnly && (
                  <span className="ml-1 text-xs px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded">Mobile</span>
                )}
              </h4>
