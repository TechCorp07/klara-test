...
export default function FHIRIntegrationPage() {
        ...
        setTimeout(() => {
          ...
          setFhirResources([
            ...
            },
            {
              ...
              status: 'active',
              intent: 'order',
              medicationCodeableConcept: { coding: [{ system: 'http://www.nlm.nih.gov/research/umls/rxnorm', code: '314076', display: 'Lisinopril 10 MG' }] },
              subject: { reference: 'Patient/123' },
          ...
          setResourceTypes([
            { id: 'Patient', name: 'Patient', description: 'Demographics and other administrative information about an individual or animal receiving care or other health-related services.' },
            { id: 'Observation', name: 'Observation', description: 'Measurements and simple assertions made about a patient, device or other subject.' },
            { id: 'Condition', name: 'Condition', description: 'A clinical condition, problem, diagnosis, or other event, situation, issue, or clinical concept that has risen to a level of concern.' },
            { id: 'MedicationRequest', name: 'Medication Request', description: 'An order or request for both supply of the medication and the instructions for administration of the medication to a patient.' },
            { id: 'Procedure', name: 'Procedure', description: 'An action that is or was performed on or for a patient. This can be a physical intervention like an operation, or less invasive like long term services, counseling, or hypnotherapy.' },
            { id: 'Immunization', name: 'Immunization', description: 'Describes the event of a patient being administered a vaccine or a record of an immunization as reported by a patient, a clinician or another party.' },
            { id: 'AllergyIntolerance', name: 'Allergy Intolerance', description: 'Risk of harmful or undesirable, physiological response which is unique to an individual and associated with exposure to a substance.' },
            { id: 'CarePlan', name: 'Care Plan', description: 'Describes the intention of how one or more practitioners intend to deliver care for a particular patient, group or community for a period of time.' }
          ]);
  ...
  const handleConnectServer = async (serverId) => {
    ...
    try {
      ...
      setFhirServers(fhirServers.map(server => {
      ...
      <div className="bg-white shadow rounded-lg mb-6">
        <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
          ...
          <p className="mt-1 max-w-2xl text-sm text-gray-500">
            Connect to healthcare systems using FHIR standards.
          </p>
        </div>
            ...
            {fhirServers.map((server) => (
              ...
              >
                <div className="px-4 py-5 sm:p-6">
                  ...
                  <p className="mt-1 text-sm text-gray-500 truncate" title={server.url}>{server.url}</p>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between items-center">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                View and manage healthcare data in FHIR format.
              </p>
            </div>
                    ...
                    <tr key={resource.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          ...
                          resource.resourceType === 'MedicationRequest' ? 'bg-purple-100 text-purple-800' : 
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">
                        ...
                        {resource.resourceType === 'Observation' && (
                          <div>
                            ...
                            <div>
                              {resource.valueQuantity.value} {resource.valueQuantity.unit} | {formatDate(resource.effectiveDateTime)}
                            </div>
                          </div>
                        )}
                        {resource.resourceType === 'Condition' && (
                          <div>
                            ...
                            <div>
                              Status: {resource.clinicalStatus.coding[0].code} | Onset: {formatDate(resource.onsetDateTime)}
                            </div>
                          </div>
                        )}
      ...
      {activeTab === 'import' && (
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
            ...
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              Import healthcare data from connected FHIR servers.
            </p>
          </div>
          <div className="px-4 py-5 sm:p-6">
            <div className="mb-6">
              ...
              {getActiveServerDetails() ? (
                <div className="bg-gray-50 p-4 rounded-md">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      getActiveServerDetails().status === 'connected' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                    }`}>
                ...
                <p className="text-sm text-gray-500">No FHIR server selected. Please select a server from the list above.</p>
              )}
            </div>
            ...
            {getActiveServerDetails() && getActiveServerDetails().status === 'connected' ? (
                  ...
                  <div className="space-y-4">
                    <div className="relative flex items-start">
                      ...
                      <div className="ml-3 text-sm">
                        ...
                        <p className="text-gray-500">Only import new or updated resources since last synchronization.</p>
                      </div>
                    </div>
                    <div className="relative flex items-start">
                      <div className="flex items-center h-5">
              ...
              <div className="text-center py-6">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Please connect to a FHIR server before importing data.
                </p>
                <div className="mt-6">
                  <button
                    type="button"
            ...
            <div className="mb-6">
              ...
              {selectedResources.length === 0 ? (
                <div className="bg-gray-50 p-4 rounded-md text-center">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Go to the FHIR Resources tab to select resources for export.</p>
                </div>
              ) : (
                <div className="bg-gray-50 p-4 rounded-md">
                  <p className="text-sm font-medium text-gray-900 mb-2">{selectedResources.length} resources selected for export</p>
                  <div className="flex flex-wrap gap-2">
                    {selectedResources.map(resourceId => {
                      const resource = fhirResources.find(r => r.id === resourceId);
                      return resource ? (
                        <span key={resourceId} className={`px-2 py-1 text-xs font-medium rounded-full ${
                          resource.resourceType === 'Patient' ? 'bg-blue-100 text-blue-800' : 
                          resource.resourceType === 'Observation' ? 'bg-green-100 text-green-800' : 
                          resource.resourceType === 'Condition' ? 'bg-yellow-100 text-yellow-800' : 
                          resource.resourceType === 'MedicationRequest' ? 'bg-purple-100 text-purple-800' : 
      ...
      {activeTab === 'settings' && (
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
            ...
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              Configure FHIR integration settings and preferences.
            </p>
          </div>
          <div className="px-4 py-5 sm:p-6">
