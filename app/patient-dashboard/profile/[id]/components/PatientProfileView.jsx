...
import { healthcare } from '@/api';
import { toast } from 'react-toastify';

/**
 * Client component for patient-profile page
 */
export default function PatientProfileClient() {
  ...
  const hasPermission = user.role === 'provider' || user.role === 'admin' || user.role === 'superadmin' || 
    ...
    <div className="container mx-auto px-4 py-8">
      {loading ? (
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg">
                      ...
                      <div className="text-xs text-gray-500">Last recorded: {patient.vitals?.recorded_at ? new Date(patient.vitals.recorded_at).toLocaleDateString() : 'Never'}</div>
                    </div>
                    
                    <div className="bg-green-50 p-4 rounded-lg">
                      <div className="text-sm text-green-600 font-medium">Heart Rate</div>
                      <div className="text-2xl font-bold">{patient.vitals?.heart_rate ? `${patient.vitals.heart_rate} bpm` : 'N/A'}</div>
                      <div className="text-xs text-gray-500">Last recorded: {patient.vitals?.recorded_at ? new Date(patient.vitals.recorded_at).toLocaleDateString() : 'Never'}</div>
                    </div>
                    
                    <div className="bg-yellow-50 p-4 rounded-lg">
                      <div className="text-sm text-yellow-600 font-medium">Temperature</div>
                      <div className="text-2xl font-bold">{patient.vitals?.temperature ? `${patient.vitals.temperature} °C` : 'N/A'}</div>
                      <div className="text-xs text-gray-500">Last recorded: {patient.vitals?.recorded_at ? new Date(patient.vitals.recorded_at).toLocaleDateString() : 'Never'}</div>
                    </div>
                    
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <div className="text-sm text-purple-600 font-medium">Oxygen Saturation</div>
                      <div className="text-2xl font-bold">{patient.vitals?.oxygen_saturation ? `${patient.vitals.oxygen_saturation}%` : 'N/A'}</div>
                      <div className="text-xs text-gray-500">Last recorded: {patient.vitals?.recorded_at ? new Date(patient.vitals.recorded_at).toLocaleDateString() : 'Never'}</div>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">Patient not found or you do not have permission to view this information.</span>
        </div>
      )}
    </div>
  );
