...
/**
 * Enhanced client component for approvals page
 * - Consolidated user approval functionality
 * - Filtering by approval type (including user registrations) and priority
 * - Detailed approval request display
 * - Action buttons for approval management
 */
export default function ApprovalsClient() {
  ...
  useEffect(() => {
    if (user && !isAdmin) {
      // Optional: redirect non-admin users
      // window.location.href = '/dashboard';
    }
  }, [user, isAdmin]);
  
  // Fetch pending approvals with filters using React Query
  const { 
    data: approvals, 
    isLoading, 
    error, 
                  ...
                  <tr key={approval.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        ...
                        {approval.type.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Showing <span className="font-medium">{approvals.results.length > 0 ? 1 : 0}</span> to <span className="font-medium">{approvals.results.length}</span> of{' '}
